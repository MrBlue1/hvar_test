...
Mutation Operator: SOR
Description: 交换乘数
Original: x * torch.rsqrt
Mutated: torch.rsqrt(variance + self.variance_epsilon) * x
"""
...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        x_norm = torch.rsqrt(variance + self.variance_epsilon) * x(variance + self.variance_epsilon)
