...
Description: 替换为LayerNorm
Original:         x_norm = x * torch.rsqrt(variance + self.variance_epsilon)
Mutated:         mean = x.mean(-1, keepdim=True); x_norm = (x - mean) * torch.rsqrt(variance + self.variance_epsilon)
"""
...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        mean = x.mean(-1, keepdim=True); x_norm = (x - mean) * torch.rsqrt(variance + self.variance_epsilon)
