...
Mutation Operator: FCR
Description: rsqrt->pow(-0.5)
Original: torch.rsqrt(variance + self.variance_epsilon)
Mutated: torch.pow(variance + self.variance_epsilon, -0.5)
"""
...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        x_norm = x * torch.pow(variance + self.variance_epsilon, -0.5)
