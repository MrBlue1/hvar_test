...
Description: 添加mean减法
Original:         variance = x.pow(2).mean(-1, keepdim=True)
Mutated:         mean = x.mean(-1, keepdim=True); variance = (x - mean).pow(2).mean(-1, keepdim=True)
"""
...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        mean = x.mean(-1, keepdim=True); variance = (x - mean).pow(2).mean(-1, keepdim=True)
